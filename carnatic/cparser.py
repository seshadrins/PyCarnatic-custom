...
def _parse_UD_digits(text):
    ...
    for c in text:
        if c == "U":
            ...
            c_note_arr.append(c)
            result += c
        elif (c == "." or c == "'" or c == "^") and len(c_note_arr) > 0 and c_note_arr[-1][-1] != c:
            c_note_arr[-1] += c
            result += c
        else:
            result += c
        ...
        while line:
            ...
            if key == "comment":
                ...
                if "Raagam:" in line or "Ragam:" in line:
                    ...
                    if total_raagas_per_search == 0:
                        # Exact match failed — try a partial (case-insensitive) search as fallback
                        raaga_ids = raaga.search_for_raaga_by_name(raagam,is_exact=False)
                        total_raagas_per_search = len(raaga_ids)
                    if total_raagas_per_search == 0:
                        warnings.warn("Raaga '"+raagam+"' not found in raaga list. Raaga will not be changed.")
                    else:
                        if total_raagas_per_search > 1:
                            warnings.warn("Multiple raagas found for '"+raagam+"'. Raaga set to the first matching one.")
...
def _w_index_mk(carnatic_note, melakartha_number):
    ...
    cn = re.sub(r"\d","",carnatic_note).strip().upper()
...
def _get_midi_note(carnatic_note):
    ...
    midi_note = re.sub(r"\d",str(octave),midi_note)
