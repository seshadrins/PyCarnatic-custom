...
# Creating menus using a QMenu object
    ...
    def _generate_lessons(self, lesson_type):
        ...
        try:
            ...
            msg_box = QMessageBox()
            msg_box.setWindowTitle("Lesson Not Available")
            msg_box.setText(
                f"The selected ThaaLa / Jaathi combination is not supported for {lesson_type.replace('_', ' ').title()}.\n\n"
                f"{e}"
            )
    ...
    def _generate_sarali_varisai(self):
        return self._generate_lessons("SARALI_VARISAI")
    def _generate_jantai_varisai(self):
        return self._generate_lessons("JANTAI_VARISAI")
    def _generate_dhaattu_varisai(self):
        return self._generate_lessons("DHAATTU_VARISAI")
    def _generate_melsthaayi_varisai(self):
        return self._generate_lessons("MELSTHAAYI_VARISAI")
    def _generate_keezhsthaayi_varisai(self):
        return self._generate_lessons("KEEZHSTHAAYI_VARISAI")
    ...
    def _play_sarali_varisai(self):
        if self._generate_sarali_varisai():
    ...
    def _play_jantai_varisai(self):
        if self._generate_jantai_varisai():
            self._play_notes_on_screen()
    def _play_dhaattu_varisai(self):
        if self._generate_dhaattu_varisai():
            self._play_notes_on_screen()
    def _play_melsthaayi_varisai(self):
        if self._generate_melsthaayi_varisai():
            self._play_notes_on_screen()
    def _play_keezhsthaayi_varisai(self):
        if self._generate_keezhsthaayi_varisai():
            self._play_notes_on_screen()
    ...
    def _play_notes_on_screen(self):
        ...
        if self._player_type == settings.PLAYER_TYPE.SCAMP:
            ...
            scamp_note_list,_,solkattu_list = cparser.parse_notation_file(_TEMP_FILE)
            self._update_ui_per_settings()
            # Write MIDI file on the main thread (fast), then play on a background
            # thread so the Qt event loop is not blocked and pause/stop work correctly.
            temp_midi_file = settings._TEMP_PATH + 'output.mid'
            cmidi.write_to_midifile_from_scamp_notes(
                scamp_note_list, temp_midi_file,
                include_percussion_layer=self._include_percussion_layer,
                solkattu_list=solkattu_list)
            self.MPlayer.is_playing = True
            def _play_in_background():
                try:
                    self.MPlayer.play_midi_file(temp_midi_file)
                except Exception as e:
                    print('ERROR during playback:', e)
                finally:
                    self.MPlayer.is_playing = False
            t = threading.Thread(target=_play_in_background, daemon=True)
            t.start()
        QApplication.processEvents()
        self._delete_file(_TEMP_FILE)
